...
class KeyMapping(object):
	...
	Color1Grab	 = QtCore.Qt.Key.Key_1
	Color2Grab	 = QtCore.Qt.Key.Key_2
	Color3Grab	 = QtCore.Qt.Key.Key_3
	Color4Grab	 = QtCore.Qt.Key.Key_4
	Training	 = QtCore.Qt.Key.Key_T
	...
	def keyPressEvent(self, event):
		...
		if controller is not None and not event.isAutoRepeat():
			...
			if key == KeyMapping.Emergency:
				...
				controller.SendLand()
			elif key == KeyMapping.Training:
				self.trained = True
				self.statusMessage = "Dectection Mode"
			elif key == KeyMapping.Color1Grab:
				qimage = QtGui.QImage(self.image.data, self.image.width, self.image.height, QtGui.QImage.Format_RGB888)
				self.color1 = qimage.pixel(320,180)
				self.statusMessage = "Color1 recorded"
				self.trained = False
				#rgb = qimage.pixel(320,180)
				#self.statusMessage = "Red: {}   Green: {}   Blue: {}".format(QtGui.qRed(rgb),QtGui.qGreen(rgb),QtGui.qBlue(rgb))
			elif key == KeyMapping.Color2Grab:
				qimage = QtGui.QImage(self.image.data, self.image.width, self.image.height, QtGui.QImage.Format_RGB888)
				self.color2 = qimage.pixel(320,180)
				self.statusMessage = "Color2 recorded"
				self.trained = False
				#rgb = qimage.pixel(320,180)
				#self.statusMessage = "Red: {}   Green: {}   Blue: {}".format(QtGui.qRed(rgb),QtGui.qGreen(rgb),QtGui.qBlue(rgb))
			elif key == KeyMapping.Color3Grab:
				qimage = QtGui.QImage(self.image.data, self.image.width, self.image.height, QtGui.QImage.Format_RGB888)
				self.color3 = qimage.pixel(320,180)
				self.statusMessage = "Color3 recorded"
				self.trained = False
				#rgb = qimage.pixel(320,180)
				#self.statusMessage = "Red: {}   Green: {}   Blue: {}".format(QtGui.qRed(rgb),QtGui.qGreen(rgb),QtGui.qBlue(rgb))
			elif key == KeyMapping.Color4Grab:
				qimage = QtGui.QImage(self.image.data, self.image.width, self.image.height, QtGui.QImage.Format_RGB888)
				self.color4 = qimage.pixel(320,180)
				self.statusMessage = "Color4 recorded"
				self.trained = False
				#rgb = qimage.pixel(320,180)
				#self.statusMessage = "Red: {}   Green: {}   Blue: {}".format(QtGui.qRed(rgb),QtGui.qGreen(rgb),QtGui.qBlue(rgb))
			else:
